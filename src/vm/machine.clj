...
(def ^:private syncthing-folders-template
  ...
  (str/join "\n"
            ["# Syncthing folders — one per line:"
             ...
             "# sync-identity."
             "#"
             "# On first sight of each folder, a default <path>/.stignore is"
             "# seeded that skips copyparty's `.hist` state dir and syncthing's"
             "# own `.stversions`. Written only when missing — edit that file"
             "# on the VM to customize ignore patterns; the reconciler won't"
             "# overwrite it."
             "#"
             "# Apply changes with:"
             "#   just upgrade <name>   (or  just sync-identity <name> on proxmox-lxc)"
...
(def ^:private traefik-dynamic-example
  ...
  (str/join "\n"
            ["# Example dynamic Traefik config. Rename this file (drop the .disabled"
             ...
             "#       rule: \"Host(`copyparty.nas.example.com`)\""
             "#       entryPoints: [websecure]"
             "#       # wg-required denies unmapped wg peers with 403 at the traefik"
             "#       # layer. Required because the nas profile configures copyparty"
             "#       # with auth-ord: idp — without a wg_users mapping there is no"
             "#       # login mechanism, so an unauthenticated request would 401"
             "#       # from copyparty anyway. Swap for wg-user if you also want to"
             "#       # serve unauthenticated content from this router (rare)."
             ...
             "#     # Defense-in-depth for ZFS snapshot directories: block any request"
             "#     # whose URL contains /.zfs/ (the ZFS control dir). The nas profile"
             "#     # already masks .zfs inside the container by bind-mounting an empty"
