...
(def ^:private mounts-template
  ...
  (str/join "\n"
            ["# LXC host bind mounts — one per line:  <host-spec>:<container-path>"
             ...
             "#                        dataset (e.g. /rpool/data/traefik/nas)."
             "#   rpool/data/nas       ZFS dataset name (auto-`zfs create`d if"
             "#                        missing; parent must exist). Whole dataset."
             "#                        Nested datasets like rpool/data/x/y are their"
             "#                        own filesystem, NOT a subdir of x — use the"
             "#                        literal-path form for real subdirectories."
             "#   local-zfs/nas        PVE zfspool storage entry (resolved to its"
             "#                        underlying dataset, then treated as above)."
             "#"
             ...
             "#   rpool/data/nas:/srv/nas                       whole dataset"
             "#   /rpool/data/traefik/nas:/srv/nas              subdir of a dataset"
             ""]))
