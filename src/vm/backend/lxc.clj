...
(defn- stage-rootfs-etc!
  ...
  (let [tmp (str (fs/create-temp-dir))
    ...
    ;; nas_hosts (per-share host scoping when combined with wireguard).
    (when (some non-empty? [(str md "/nas_passwd") (str md "/nas_acl")
                            (str md "/nfs_clients") (str md "/nas_hosts")])
...
(defn- rootfs-perm-cmds [root]
  [(format "chmod 0644 %s/etc/hostname 2>/dev/null || true" root)
   ...
   (format "chmod 0644 %s/etc/nas/nas_acl %s/etc/nas/nfs_clients %s/etc/nas/nas_hosts 2>/dev/null || true" root root root)
...
(defn- finish-create! [this cfg name]
  (let [profile (or (machine/read-field cfg name "profile") "core")
        var-size (or (machine/read-field cfg name "disk_size")
                     (machine/read-field cfg name "var_size") "8G")]
