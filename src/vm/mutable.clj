...
(defn generate-mutable-flake
  ...
  (let [imports (apply str (map #(str "          ./profiles/" % ".nix\n")
    ...
    (str "{\n"
         ...
         "          # modules/image-version.nix requires this to be provided.\n"
         "          # The stock image bakes it via the main flake at build time;\n"
         "          # when nixos-rebuild runs from /etc/nixos on the VM, we don't\n"
         "          # know the original build's version, so we just note that.\n"
         "          imageVersion = \"unknown (rebuilt from /etc/nixos)\\n\";\n"
         "        };\n"
         "        modules = [\n"
