...
# Push this repo, bump its rev in ../sway-home's flake.lock, then commit+push sway-home.
push-sway-home sway_home="../sway-home":
    ...
    cd "{{sway_home}}"
    [ -z "$(git status --porcelain)" ] || { echo "sway-home has uncommitted changes; commit first"; exit 1; }
    git pull --ff-only
    nix flake update nixos-vm-template
    if [ -n "$(git status --porcelain flake.lock)" ]; then
      git add flake.lock
      git commit -m "flake update nixos-vm-template"
      git push
    else
      echo "sway-home flake.lock already up to date"
      if [ "$(git rev-list @{u}..HEAD --count 2>/dev/null || echo 0)" -gt 0 ]; then
        git push
      fi
    fi
