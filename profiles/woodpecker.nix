# Woodpecker CI agent profile
#
# Runs a Woodpecker agent as a native systemd service.
# The backend is auto-detected from the profile: docker if the docker
# profile is included, otherwise local.
#
...
in
{
  config = {
    services.woodpecker-agents.agents.exec = {
      enable = true;
      environment = {
        WOODPECKER_BACKEND = if dockerEnabled then "docker" else "local";
        WOODPECKER_HEALTHCHECK = "true";
        WOODPECKER_GRPC_SECURE = "true";
        WOODPECKER_AGENT_CONFIG_FILE = "/var/lib/woodpecker/agent.conf";
        # The local backend creates its per-pipeline workspace under TMPDIR.
        # Default /tmp is a small RAM-backed tmpfs (size=50%), which overflows
        # when building/exporting large VM images. Point it at /var instead.
        TMPDIR = "/var/lib/woodpecker/tmp";
      };
    ...
    users.users.woodpecker = {
      ...
      shell = pkgs.bash;
      extraGroups = lib.optionals dockerEnabled [ "docker" ];
    };
    ...
    systemd.tmpfiles.rules = [
      ...
      "d /var/lib/woodpecker/tmp 0700 woodpecker woodpecker -"
    ];
