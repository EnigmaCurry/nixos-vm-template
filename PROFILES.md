...
| **cloud-init** | Enables cloud-init + qemu-guest-agent so the built image can be used as a Proxmox template (`qm template`) and cloned per-VM without going through this repo's identity-injection flow. **Requires `mutable` mode** — cloud-init writes to `/etc/hostname`, `/etc/ssh/ssh_host_*_key`, systemd-networkd configs, etc., which are read-only in `immutable` and `semi-mutable`. If you leave the mode unset, `cloud-init` auto-implies `mutable`; combining with `semi-mutable` is rejected at build time. |
