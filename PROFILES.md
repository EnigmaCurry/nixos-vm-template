...
| **nftables** | Replaces the default iptables firewall with an nftables ruleset loaded from `/var/identity/nftables.conf`. Paired with PCI NIC passthrough on Proxmox (see [PROXMOX.md](PROXMOX.md#gpu-passthrough) — same mechanism, the wizard offers a NIC picker instead of a GPU picker). Intended for router/firewall VMs. Any `tcp_ports`/`udp_ports` files in the machine dir are ignored while this profile is active. |
