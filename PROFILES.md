...
| **wireguard** | WireGuard VPN on `wg0`, config in `machines/<name>/wireguard.conf` (mode 0600) — synced at create/upgrade, private key pinned in the file so recreate preserves the tunnel identity. `just create` seeds a fresh keypair and commented peer template; edit to add `[Peer]` sections and run `just sync-identity <name>` to apply. Opens UDP 51820 by default. |
