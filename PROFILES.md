...
| **wireguard** | WireGuard VPN on `wg0`, config in `machines/<name>/wireguard.conf` (mode 0600) — synced at create/upgrade, private key pinned in the file so recreate preserves the tunnel identity. `just create` seeds a fresh keypair and commented peer template; edit to add `[Peer]` sections and run `just upgrade <name>` to apply. Opens UDP 51820 by default. This is the **client / spoke** role — no IP forwarding. See [WIREGUARD.md](WIREGUARD.md) for a hub-and-spoke walkthrough (nas + Samba over VPN). |
| **wireguard-hub** | **Server / hub** role for WireGuard — enables IPv4 forwarding so spoke-to-spoke traffic transits this peer. Layer on top of `wireguard` (profile list contains BOTH `wireguard` AND `wireguard-hub`); do **not** add to roaming clients. |
